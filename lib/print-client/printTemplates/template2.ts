...
export const generatePrintTemplate2 = async (
    data: ReceiptData,
    ...
    encoder.doubleLine(settings.charsPerLine).newline();


    encoder.align('left');
    encoder.leftRight('Receipt #:', data.receiptNumber, settings.charsPerLine).newline();
    if (data.transactionId) {
        encoder.leftRight('Transaction ID:', data.transactionId, settings.charsPerLine).newline();
    }
    ...
    encoder.leftRight('Payment Method:', data.paymentMethod, settings.charsPerLine).newline();
    ...
    encoder.newline().align('center').text('Thank You for Your Business!').newline();
